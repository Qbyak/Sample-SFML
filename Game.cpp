...
void Game::play()
{
	...
	while (window.isOpen())
	{
		...
		update_all(gracz, elapsed); // updatowanie pozycji platform oraz bomb nastepnie rysowanie ich		
        draw_all(window); // rysowanie wszystkich obiektow poza graczem 
		gracz.update(window, platformy, bomby); // update gracza na podstawie pozycji platform i innych rzeczy nastepnie rysowanie go 
...
void Game::next_screen(player &play, const sf::Time& elapsed) // funkcja rusza wszystkie elementy na ekranie w zaleznosci od wysokosci gracza 
{
		...
		if (play.getPosition().y < 300) // zabezpieczenie aby gracz nie wyskoczyl poza ekran 
		{

			move_all((sf::Vector2f(0, 300 - play.getPosition().y)));
			play.move(sf::Vector2f(0, 300 - play.getPosition().y));
		}
...
void Game::update_all(player& gracz, const sf::Time& elapsed)
{
	for (auto& x : platformy) // updatowanie pozycji platform oraz rysowanie ich
	{
		x->update();
	}
	next_screen(gracz, elapsed); // updatowanie pozycji bomb oraz platform 
}
...
void Game::death(player& gracz, sf::RenderWindow& window) // ekran smierci 
{
	for (auto x : bomby)
	{
		if (gracz.getGlobalBounds().intersects(x->getGlobalBounds()))
		{
			sf::Clock zegar;
